...
auto defaultInjector = [](di::extension::detail::scoped& scope, QApplication& qApplication, ICommandLineArguments& commandLineArguments) {
    return di::make_injector(
        ...
        di::bind<IContextProperties>().to<ContextProperties>(),
        di::bind<IPlayer>().to<CurrentPlayer>(),
        di::bind<IFactory<ILocalSocket>>().to(Factory<LocalSocket> { }),
        di::bind<IFactory<ILocalServer, QString>>().to(Factory<LocalServer> { }),
        di::bind<IStreamingServiceLoader>().to<StreamingServiceLoader>(),
        di::bind<IStreamingServiceWatcher>().to<StreamingServiceWatcher>(),
        di::bind<ILoggerFactory>().to<SpdLoggerFactory>(),
        di::bind<IAlbumArtDownloader>().to<AlbumArtDownloader>(),
        di::bind<ILocalAlbumArt>().to<LocalAlbumArt>(),
        di::bind<IHotkeys>().to<Hotkeys>(),
        di::bind<ISystemTrayIcon>().to<SystemTrayIcon>(),
        di::bind<IListeningHistoryDatabase>().to<SqlLiteListeningHistoryDatabase>(),
        di::bind<ISettingsStore>().to<QSettingsStore>(),
        di::bind<IWorkDispatcher>().to<QtConcurrentWorkDispatcher>(),
        di::bind<ISettingsSchemaLoader>().to<SettingsSchemaLoader>(),
        di::bind<IStreamingServiceCreator>().to<StreamingServiceCreator>(),
        di::bind<IThemeLoader>().to<ThemeLoader>(),
        di::bind<ILatestRelease>().to<LatestBinTrayRelease>(),
        di::bind<IHttpClient>().to<HttpClient>(),
        di::bind<IFileDownloader>().to<FileDownloader>(),
        di::bind<IUserScriptFactory>().to<UserScriptFactory>(),
        di::bind<IMainWindow>().to<MainWindowViewModel>(),
        di::bind<INotifications>().to<Notifications>(),
        di::bind<INetworkProxies>().to<NetworkProxies>(),
        di::bind<ITimer>().to<Timer>().in(di::unique),
        di::bind<IListeningHistory>().to<DelayedListeningHistory>(),
